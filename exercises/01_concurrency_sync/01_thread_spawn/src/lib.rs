...
pub fn parallel_sum(a: Vec<i32>, b: Vec<i32>) -> (i32, i32) {
    let handle_a = std::thread::spawn(move || a.into_iter().sum::<i32>());
    let handle_b = std::thread::spawn(move || b.into_iter().sum::<i32>());
...
pub fn handle_panic(value: i32, should_panic: bool) -> Result<i32, ()> {
    println!("value: {}, should_panic: {}", value, should_panic);
